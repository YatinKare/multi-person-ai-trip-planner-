    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(
        (trip) =>
          ...
          (trip.rough_timeframe &&
            trip.rough_timeframe.toLowerCase().includes(query)),
      )
    }
...
<div class="max-w-7xl mx-auto">
  ...
  >
    <div>
      ...
      <p class="mt-1 text-base-content/60">
        Manage your group adventures and itineraries.
      </p>
    </div>
    <button
  ...
  {#if data.trips.length === 0}
    ...
    >
      <div class="max-w-md">
        <div class="mb-6">
          <span class="material-symbols-outlined text-9xl text-base-content/20"
            >flight_takeoff</span
          >
        </div>
        <h3 class="text-2xl font-bold mb-3">No trips yet?</h3>
        <p class="text-base-content/60 mb-6">
          Adventure is just a click away. Start planning your next group getaway
          in seconds.
        </p>
        <button
          ...
          <div class="grid grid-cols-1 gap-4">
            <div class="card bg-base-200 border border-base-300">
              <div class="card-body items-center text-center gap-3 p-4">
                <div
                  class="w-10 h-10 rounded-lg bg-base-300/50 flex items-center justify-center text-primary"
                >
            ...
            <div class="card bg-base-200 border border-base-300">
              <div class="card-body items-center text-center gap-3 p-4">
                ...
                <div>
                  ...
                  <p class="text-xs text-base-content/60">
                    Sync up with friends instantly
                  </p>
                </div>
              </div>
            </div>
  ...
  {:else}
    <!-- Filters and Search -->
    <div
      class="mb-8 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"
    >
      <!-- Search Bar -->
      ...
      <div class="flex flex-wrap gap-2">
        <button
          class="btn btn-sm {selectedFilter === 'all'
            ? 'btn-neutral'
        ...
        <button
          class="btn btn-sm {selectedFilter === 'collecting'
            ? 'btn-neutral'
            : 'btn-ghost bg-base-300'}"
        ...
        <button
          class="btn btn-sm {selectedFilter === 'voting'
            ? 'btn-neutral'
            : 'btn-ghost bg-base-300'}"
        ...
        <button
          class="btn btn-sm {selectedFilter === 'planning'
            ? 'btn-neutral'
            : 'btn-ghost bg-base-300'}"
        ...
        <button
          class="btn btn-sm {selectedFilter === 'finalized'
            ? 'btn-neutral'
            : 'btn-ghost bg-base-300'}"
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
      {#each filteredTrips as trip (trip.id)}
        <div
          class="card bg-base-200 border border-base-300 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all overflow-hidden group"
        >
          <figure class="relative aspect-video">
            ...
            <button
              class="btn btn-circle btn-ghost btn-sm absolute right-3 top-3 z-10 bg-black/40 backdrop-blur-md"
            >
              <span class="material-symbols-outlined text-lg">more_horiz</span>
            </button>
            <!-- Placeholder gradient for trip image -->
            <div
              class="flex h-full w-full items-center justify-center bg-gradient-to-br from-base-300 to-base-100"
            >
          ...
          <div class="card-body p-5">
            ...
            >
              <div class="text-sm text-base-content/60">
                ...
                >
                {trip.member_count}
                {trip.member_count === 1 ? "member" : "members"}
              </div>
              <a
                href="/trips/{trip.id}"
      ...
      >
        <div class="card-body items-center justify-center text-center">
          <div
            class="w-16 h-16 rounded-full bg-base-300 flex items-center justify-center group-hover:bg-primary/20 group-hover:text-primary transition-colors"
          >
            <span class="material-symbols-outlined text-3xl">add</span>
          </div>
          <h3 class="card-title text-lg mt-4 group-hover:text-primary">
            Plan a New Trip
          </h3>
