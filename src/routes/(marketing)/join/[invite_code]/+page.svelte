...
<div class="relative flex min-h-screen w-full flex-col overflow-hidden" data-theme="tripsync">
	...
	<!-- Navbar -->
	<header class="relative z-20 flex items-center justify-between px-6 py-4 md:px-10 lg:px-40">
		<a href="/" class="flex items-center gap-2 cursor-pointer">
			<div class="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-base-300">
				<span class="material-symbols-outlined text-xl font-bold">flight_takeoff</span>
			</div>
			<h2 class="text-white text-xl font-bold tracking-tight">TripSync</h2>
		</a>
		{#if !data.session}
			<a href="/login" class="hidden md:flex btn btn-ghost btn-sm text-white hover:bg-white/10">
				Sign In
			</a>
		{/if}
	</header>
	...
	<!-- Main Content Area -->
	<main class="relative z-20 flex flex-1 flex-col items-center justify-center px-4 py-10 md:px-6">
			...
			>
				...
				<div class="flex flex-col gap-6 px-6 pb-8 pt-2">
					...
					<div class="text-center">
						<p class="mb-1 text-sm font-medium uppercase tracking-wider text-base-content/60">
							You're invited to
						</p>
						<h1 class="text-3xl font-black leading-tight tracking-tight text-white md:text-4xl">
							{data.trip.name}
						</h1>
						{#if tripTimeframe()}
							<div class="mt-2 flex items-center justify-center gap-2 text-sm text-base-content/60">
							...
							<div class="avatar">
								<div class="w-20 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
									{#if data.trip.organizer?.avatar_url}
										<img src={data.trip.organizer.avatar_url} alt={data.trip.organizer.full_name || 'Organizer'} />
									{:else}
							...
							<div class="flex flex-col items-center justify-center gap-3 md:flex-row md:justify-between">
											...
											<div class="w-10">
												...
												{:else}
													<div class="w-10 h-10 rounded-full bg-neutral flex items-center justify-center">
														<span class="material-symbols-outlined text-sm text-neutral-content">person</span>
													</div>
												{/if}
											</div>
										</div>
								...
								<p class="text-sm font-medium text-white/90">
									{#if data.otherMembers.length > 0}
										Join {data.otherMembers[0]?.full_name?.split(' ')[0] || 'others'}
										{#if data.memberCount > 1}
											& {data.memberCount - 1} {data.memberCount === 2 ? 'other' : 'others'}
										{/if}
									{:else}
					...
					<div class="flex flex-col gap-3">
						<form method="POST" action="?/joinTrip" use:enhance={() => {
							isSubmitting = true;
							return async ({ update }) => {
								await update();
								isSubmitting = false;
						...
						<div class="text-center">
							<span class="text-sm text-base-content/60">Already have an account? </span>
							<a
