	...
	const handleSubmit: SubmitFunction = () => {
		...
		return async ({ result, update }) => {
			...
			if (result.type === 'success') {
				...
				errorMessage = result.data?.message || 'Failed to leave trip. Please try again.';
			}
			await update();
		};
	};
	...
	>
		...
		>
			<div class="p-8">
				...
				<div class="mt-4 h-1 w-full bg-gradient-to-r from-warning/0 via-warning/50 to-warning/0 rounded-full"></div>
			</div>
		</div>
	</div>
{/if}
